...
import com.nhom7.web.repository.BookRepository;
...
public class BookService {
	...
	public Book updatecount(Book newBook, Order order, OrderDetail orderDetail, HttpServletRequest request) throws IOException{
		Book updateBook;
		updateBook = newBook;
		if (newBook.getId() != null ){
			if (order.getStatus().equals("Duyệt")){
				updateBook = bookRepository.findById(newBook.getId()).get();
				updateBook.setNumber_in_stock(newBook.getNumber_in_stock()- orderDetail.getNumber());
				updateBook.setNumber_sold(newBook.getNumber_sold() + orderDetail.getNumber());
			}
